...
colunas=arq.columns[:34];#titulos nomes das colunas antes da classe
#Normalizando dados
for col in colunas:
    menor=arq[col].min();
    maior=arq[col].max();
...
txApr=0.01;#taxa de aprendizado
...
for cont in range(0,Nrodadas):
    ...
    treinamento=arq.sample(frac=0.8);#dividindo a amostra em 80% para treinamento
    teste=(arq.drop(treinamento.index)).values;#amostra para teste
